...
rebuild) to eliminate today's staleness — micro-edit, no code change. The **systemic** gap
...
| 3.1 | Non-bypassable envelope verification (scoped) | HOLDS as scoped, but see MUST-FIX-2 for an adjacent unscoped gap (API auth) that the claim's own scope clause doesn't cover | — | — |
