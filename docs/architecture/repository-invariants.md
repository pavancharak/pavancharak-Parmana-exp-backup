...
1. `packages/execution-gateway/tests/unit/public-api-boundary.test.ts` (Phase 1D) — imports the built public entry point at runtime and asserts a named list of 10 internal symbols is absent (`Object.prototype.hasOwnProperty`), and that the 3 factories + `ExecutionGateway` are present.
