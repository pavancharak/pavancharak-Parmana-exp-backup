...
| `MockConnector` implementing `Connector` in `connector-sdk/src` | `packages/connector-sdk/src/MockConnector.ts` | Explicit test double; never constructed by production bootstrap (confirmed: no `packages/api/src/bootstrap/*.ts` file references it) |
