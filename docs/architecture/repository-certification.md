...
| ID | Status | Severity | Description | Location | Rationale | Recommended phase |
|---|---|---|---|---|---|---|
| TD-1 | **Closed** (2026-08-05, Phase 2A.4 — see `docs/operations/td1-closure-summary.md`) | **High** | `vendor-payment` connector is permanently backed by `MockConnector`, unconditionally registered (unlike credential-gated Razorpay/HubSpot), fabricating signed `{success:true}` Trust Records/Receipts for `payments:execute` with no real vendor ever contacted. | `packages/api/src/bootstrap/createVendorPaymentConnector.ts`, `createConnectorRegistry.ts` | Self-documented as an intentional interim stand-in ("until the real enterprise connector is introduced"), but no gating or evidentiary marking distinguishes its output from a genuine execution. | Phase 2: implement the real connector, or gate registration behind explicit configuration (mirroring Razorpay/HubSpot's fail-closed pattern), or mark its Trust Records as non-authoritative. |
| TD-2 | **Closed** (2026-08-05, Phase 2C — see `docs/architecture/phase2c-terminology-guard.md`) | **High** | CI's retired-terminology guard fails against the current tree: `docs/site/changelog.mdx:93` legitimately narrates repo terminology history but isn't in the guard's exclusion list. | `.github/workflows/ci.yml` (guard step), `docs/site/changelog.mdx` | Structurally identical, already-solved case exists for `ROADMAP-v1.md`; this file was simply missed. | Immediate/next phase: add `changelog.mdx` to the exclusion list (or move the historical note into an already-exempted file). |
| TD-3 | Open | **Medium** | No CI-enforced code coverage threshold; `npm run coverage` exists but is never invoked by `ci.yml`. | `.github/workflows/ci.yml`, `package.json` | Coverage regressions in any package are currently silent. | Phase 2: add a coverage step/threshold gate. |
| TD-4 | Open | **Low** | `connector-sdk` ships five vendor-named wrapper files (`WorkdayConnector.ts`, `VendorPaymentConnector.ts`, `SapConnector.ts`, `SalesforceConnector.ts`, `OracleConnector.ts`) that all just return a `MockConnector` instance; none are mentioned in `system-architecture.md`'s package responsibility table. | `packages/connector-sdk/src/connectors/*` | Not a boundary violation (still `MockConnector` under the hood), but an undocumented surface. | Next documentation pass: either document them explicitly as example/demo connector stubs, or remove if unused. |
| TD-5 | Open | **Low** | `system-architecture.md` §8's dependency graph omits `storage` and `replay` packages, and omits `runtime`'s reference to `storage`. | `docs/architecture/system-architecture.md` §8 | Incomplete, not incorrect — the graph reads as a summary of pipeline-central packages. | Next documentation pass, if the graph's intended scope is exhaustive. |
| TD-6 | Open | **Low** | `EXECUTION-FLOW-AUDIT.md` references `policies/vendor-payment/1.0.0/policy.json`; only `2.0.0` exists today. | `docs/architecture/EXECUTION-FLOW-AUDIT.md` | Stale path in a point-in-time historical audit record, not a living document. | No action needed unless the file is promoted to living-doc status. |
| TD-7 | Open | **Low** | `packages/api/src/bootstrap/createVendorPaymentSecureConnector.ts` is a committed, 0-byte file, dead since introduction (`ddf4bc5`), referenced by nothing. | `packages/api/src/bootstrap/createVendorPaymentSecureConnector.ts` | Pure cosmetic cruft; no functional impact (nothing imports it). | Any future cleanup pass: delete. |
| TD-8 | Open | **Low** | `receipt`'s `package.json` doesn't declare `@parmana/execution-control`/`@parmana/execution-system` even as `devDependencies`/`peerDependencies`, relying solely on `tsconfig.json` project references for its type-only import. | `packages/receipt/package.json` | Works today only because npm workspaces hoist every package; would silently break if `receipt` were ever extracted/published standalone. | Before any standalone package extraction. |
| TD-9 | Open | **Low** | `ExecutionRequestBuilder.build()` manually re-lists `businessTransactionId`/`action`/`target`/`parameters` instead of calling the shared `toExecutableContent()` helper `RuntimeEngine.ts` already uses for the same four fields. | `packages/runtime/src/ExecutionRequestBuilder.ts` | No drift today (both derive identical values), but nothing enforces they can't diverge — this pair feeds `ExecutionGateway`, a trust-critical boundary. | Next runtime-package touch: one-line change to call `toExecutableContent()`. |
| TD-10 | Open | **Low** | `typescript/` client SDK: 5 of 9 test files remain empty stubs (`HealthApi.test.ts`, `ParmanaClient.test.ts`, `PolicyApi.test.ts`, `ReplayApi.test.ts`, `VerificationApi.test.ts`); 4 were filled in and a new integration test added since first found (2026-07-07), unchanged since Phase 2B (2026-08-05). | `typescript/test/*.test.ts` | Published client SDK, partially covered. | Session-sized: write real coverage for the 5 remaining surfaces. |
| TD-11 | Open | **Low** | `ExecutionEvidenceComponent.execute()` is a no-op (`return context;` unchanged) with a `// TODO: Build ExecutionEvidence...` comment; confirmed unreferenced anywhere outside its own file. | `packages/runtime/src/components/ExecutionEvidenceComponent.ts` | Dead pipeline stage, not wired into any live `RuntimePipeline` composition. | Any future cleanup pass: finish it or delete it. |
| TD-12 | Open | **Low** | `OverrideService`/`OverrideVerifier` are unreachable from any route — zero references from `packages/api/src/**`. | `packages/runtime/src/policy/OverrideVerifier.ts`, `packages/runtime/src/services/override-service.ts` | Guarded/intentionally parked capability, not a boundary violation. | No action needed unless this capability is scheduled for exposure. |
| TD-13 | Open | **Medium** | `POST /replay` exists and returns 200, but `ExecutionTrustApplication.replay()` does not invoke `@parmana/replay`'s `ReplayExecutor`/`ReplayPipeline` at all — it re-fetches the Trust Record and re-verifies its hash/signature, functionally duplicating `/verify`. `@parmana/replay`'s own "deterministic reconstruction" machinery remains entirely unreachable from any route. | `packages/runtime/src/ExecutionTrustApplication.ts:131-160` (the route handler), `packages/replay/src/*` (the unreachable package) | Refined this phase from the prior, vaguer framing ("Replay unreachable from /replay") — the route is not missing, it does something materially different from what the package/route name and `docs/CLAIMS.md` 2.7 imply. Not a security or trust-fabrication issue (nothing is misrepresented as more-verified than it is), but a claims-precision gap. | Next phase touching `/replay`: either wire in the real package, or correct the route's documented behavior/name to match what it actually does. |
| TD-14 | Open | **Low** | `LedgerSerializer`'s `JSON.stringify(entry, Object.keys(entry).sort())` uses a flat key allowlist applied at every recursion level, so any nested object whose keys don't also appear at the top level serializes as `{}`. | `packages/storage/src/ledger/LedgerSerializer.ts:9` | Confirmed dead code (same orphaned storage/ledger subsystem as TD-12/TD-13's territory) — the bug is real but nothing live exercises it. | No action needed unless this subsystem is revived. |
| TD-15 | **Closed** (2026-08-05, Phase 2D — see `docs/architecture/phase2d-execution-failure-testing.md`) | Low (was: untested-but-parked) | `execution-failure.integration.test.ts` was permanently `describe.skip`'d with a stated blocker ("RuntimeFactory always creates a DefaultExecutionSystem internally") that Phase 2B found already resolved, and that Phase 2D independently confirmed — plus found and fixed a second, deeper problem the DI fix alone didn't touch: the test never used the DI seam, and its assertions described a response shape (`response.body.execution.status`) that has never matched this API's actual, intentional no-leak error contract. | `packages/api/tests/integration/execution-failure.integration.test.ts` | Restored using `createInspectableExecutionSystem` (an existing helper), with assertions verified empirically against real current behavior, plus new coverage (Trust Record/Receipt/Refusal-Record absence, exactly-once connector invocation, determinism) beyond the original scope. | — |
| TD-16 | Open | **Medium** | `ReceiptCrypto` and `SettlementConfirmationCrypto` each directly instantiate their own `FileKeyProvider()`, bypassing the "crypto composition root" (`CryptoBootstrap`) the rest of the system uses; both self-documented as temporary. Live in production (`ReceiptService` constructs `ReceiptCrypto` directly). | `packages/crypto/src/ReceiptCrypto.ts`, `packages/crypto/src/SettlementConfirmationCrypto.ts` | Not exploitable today (`FileKeyProvider`/`KEY_PROVIDER=local` is the only implemented key provider anywhere in the repo) — a latent gap that would activate silently the moment a KMS/HSM provider (roadmap Move 1) is implemented and everything else migrates but these two classes don't. | Sequence with roadmap Move 1 (KMS/HSM custody), not before — premature to build the seam for a provider that doesn't exist yet. |
| TD-17 | Open | **Low** | `createGatewayIdentity.ts`/`createSessionStore.ts` carry `TODO: Replace ... with your production gateway identity / production authentication mechanism` comments that read as unresolved security gaps but, traced fully, are not: the identity string is a non-secret label (real auth is `SignedTokenConnectorAuthenticator`'s cryptographic signature check), and the session-issuance "authentication" value is checked by in-process object-reference equality, not content, so its value is architecturally inert. | `packages/api/src/bootstrap/createGatewayIdentity.ts`, `createSessionStore.ts` | Misleading comments, not a real security gap — independently traced end-to-end (Phase 2B). | Next bootstrap touch: reword the comments to stop reading as open security work. |
| TD-18 | Open | **Low** | `GET /transactions` filters caller-scoped results after pagination is applied, not by pushing the scope into the storage query — a page can under-fill (never over-disclose) when other callers' rows occupy its window. Self-documented as an accepted trade-off. | `packages/api/src/routes/transactions.ts:58-64` | Explicitly not a security gap by the route's own comment; a pagination-correctness nuisance only. | Low priority; revisit if/when caller-scoped pagination correctness becomes a real user complaint. |
| TD-19 | Open | **Low** | 9 debug `console.log` statements sit in the core request/execution hot path, unconditional (no log-level or `NODE_ENV` gate), firing on every request: `"[ROUTE] before/after execute"` (2, `execute.ts`) and numbered `"[APP] 1"`–`"[APP] 7"` stage markers spanning the entire `ExecutionTrustApplication.execute()` method. Introduced 2026-07-18 (`a6b9d8b`, "Execution Integrity Platform v0.3"); never previously flagged by any prior audit (Phase 1H's certification, `GAP-AUDIT.md`, or Phase 2B's assessment). | `packages/api/src/routes/execute.ts:108,115`, `packages/runtime/src/ExecutionTrustApplication.ts:68,73,78,83,88,94,102` | Debug instrumentation left behind after a development session; no functional or security impact, but noise in every production log line for the system's most central method. | Next touch to either file: delete all 9 (or replace with a real, level-gated logger if execution tracing is actually wanted). |
| TD-20 | Open | **Low** | A second, entirely unused "composition root" for Execution Control exists alongside the real one: `ExecutionControlComposition.ts` and `ConnectorFactory.ts` (100 lines combined) have zero references from any other file in the repository. Matches `docs/investigations/GAP-AUDIT.md`'s G-13 finding (2026-07-07) precisely, still unaddressed a month later and never previously added to this register. | `packages/api/src/bootstrap/ExecutionControlComposition.ts`, `packages/api/src/bootstrap/ConnectorFactory.ts` | Confusion risk for a new contributor or auditor who reads the wrong file as source of truth (the real chain is `createExecutionControl.ts`/`createConnectorRegistry.ts`) — zero functional risk since nothing constructs or imports either dead class. | Any future cleanup pass: delete both (already confirmed zero references; low-risk removal). |
| TD-21 | Open | **Low** | `docs/CLAIMS.md:835` still describes `create-connector-registry.test.ts` as asserting "vendor-payment remains resolvable when razorpay is not" — true before Phase 2A, false since: that exact test was changed to assert the opposite (fails closed) outside `NODE_ENV=test`. Flagged during Phase 2A/2A.1 but never corrected. | `docs/CLAIMS.md:835` | Stale claim about current test behavior in an actively-referenced claims document (not a dated historical snapshot like `EXECUTION-FLOW-AUDIT.md`/TD-6). | Next `CLAIMS.md` touch: correct the one clause; out of scope for a dedicated phase on its own. |

No item above is classified **Critical**, and none of TD-3 through TD-21 constitutes an active bypass of the tested architectural boundaries (single pipeline, adapter ownership, call-site allowlist, import boundaries all held under falsification attempts, re-confirmed as recently as Phase 2E). TD-1 and TD-2 were **High** because they represented, respectively, a live production-trust gap and a currently-broken CI gate — both real and actionable, neither an architectural-boundary breach; both are now closed. TD-13 and TD-16 are the only open items rated **Medium** (§ their rows above for why); everything else open is **Low**.

**TD-1 closure note (2026-08-05, added Phase 2A.4 — this is an addendum, not a revision of the finding above, which remains the accurate record of what this certification pass found on its original date):** TD-1 is closed. The finding, severity, description, location, and rationale above are preserved exactly as originally certified — they were accurate when written and remain the historical record of the issue. Closure rests on a four-phase evidence chain, each independently verified: Phase 2A implemented the fix (commit `6eff8ec`); Phase 2A.1 conducted an independent operational readiness review before deployment; Phase 2A.2 deployed the exact reviewed commit to production and confirmed the fix active via direct production log evidence; Phase 2A.3 resolved the one remaining uncertainty (historical database identity) via self-triggered, timestamp-correlated production evidence and found zero non-test-attributed historical records. Full evidence chain, commit SHAs, and dates: `docs/operations/td1-closure-summary.md`.

**TD-2 closure note (2026-08-05, added Phase 2C):** TD-2 is closed. Root cause (the exclusion list's inherent tendency to fall behind every new document that legitimately narrates the retired term) diagnosed and the list completed, re-organized by category, with guidance for future maintainers to extend it correctly rather than needing another dedicated repair phase. Full evidence: `docs/architecture/phase2c-terminology-guard.md`.

**TD-15 addition and closure note (2026-08-05, added Phase 2E, closed Phase 2D):** TD-15 was never added to this table before Phase 2E — it existed only in `docs/architecture/phase2b-technical-debt-assessment.md`'s own, separate register. Added here retroactively, already closed, for a single canonical source of truth. Full evidence: `docs/architecture/phase2d-execution-failure-testing.md`.

**TD-9 through TD-14 and TD-16 through TD-18 provenance note (added Phase 2E):** first identified in `docs/architecture/phase2b-technical-debt-assessment.md` (2026-08-05) and never previously merged into this canonical register. Each was independently re-verified against current source during Phase 2E, not copied — TD-13 in particular was refined (the `/replay` route exists and returns 200, it does not silently 404; the gap is that it doesn't invoke `@parmana/replay`'s actual machinery). TD-19, TD-20, and TD-21 are new findings from Phase 2E itself. Full evidence and risk scoring: `docs/architecture/phase2e-engineering-debt-assessment.md`.
...
| Execution ownership | **Certified**, with a documented production-integrity caveat that does not implicate the architectural boundary itself (§4.2, §4.9, TD-1) — **caveat closed 2026-08-05, Phase 2A.4; see `docs/operations/td1-closure-summary.md`** |
...
## Addendum (2026-08-05, Phase 2A.4)

This certification report is preserved exactly as originally published — nothing above this addendum has been rewritten, and its findings remain an accurate, point-in-time record of the repository's state as of Phase 1H. Repository status has changed since that publication date: **TD-1 has since been closed**, through a four-phase program (Phase 2A implementation, Phase 2A.1 operational readiness review, Phase 2A.2 production deployment verification, Phase 2A.3 historical integrity verification) independently verified at each stage. The complete evidence chain, commit SHAs, and dates are recorded canonically in `docs/operations/td1-closure-summary.md`; this addendum exists only to point there, not to restate that evidence. TD-2 through TD-8 are unchanged and remain open, exactly as this report originally classified them.
