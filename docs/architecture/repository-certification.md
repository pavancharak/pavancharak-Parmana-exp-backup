...
No item above is classified **Critical** — nothing found constitutes an active bypass of the tested architectural boundaries (single pipeline, adapter ownership, call-site allowlist, import boundaries all held under falsification attempts). TD-1 and TD-2 were/are **High** because they represent, respectively, a (now-closed) live production-trust gap and a currently-broken CI gate — both real and actionable, neither an architectural-boundary breach.
