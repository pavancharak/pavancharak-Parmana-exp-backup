...
<Info>**[AVAILABLE]**. Every row below was triggered against a live server this session or the session that produced [REST API Introduction](/api-reference/introduction), not inferred from reading source. See [Error handling](/api-reference/error-handling) for the envelope shape this table assumes.</Info>
