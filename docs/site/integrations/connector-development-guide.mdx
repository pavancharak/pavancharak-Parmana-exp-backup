...
the default *production* server registers zero connectors out of the box today —
`vendor-payment` (`packages/api/src/bootstrap/createVendorPaymentConnector.ts`) used to
register a `MockConnector` unconditionally, including in production; that was a trust gap
(a `payments:execute` request would get a genuinely signed success Trust Record with no real
vendor ever contacted) and has been closed. `payments:execute` now fails closed in
production, the same as any other unregistered capability. The registration pattern below
still applies once a real implementation exists, or when `NODE_ENV=test` (where
`createVendorPaymentConnector()` still returns a `MockConnector`, exactly as before, for
unit/integration tests and explicit demo/local runs):

```typescript
// packages/api/src/bootstrap/createConnectorRegistry.ts (abridged, real source)
const registry = new ConnectorSdkRegistry();
const connector = createVendorPaymentConnector(); // undefined outside NODE_ENV=test today
...
Adding a connector to the running server means editing three files:
...
in production until a real implementation exists, exactly like this pattern and like
`createRazorpayCredentialProvider.ts`/`createHubSpotCredentialProvider.ts` already do), and
...
and HubSpot are the two connectors actually reachable in the default production server, see
