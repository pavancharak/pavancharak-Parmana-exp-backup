...
<Warning>
**No way to send the caller bearer key yet.** Checked directly: `ParmanaClient.__init__`
(`python/parmana/client.py`) has no `api_key` parameter, and `HttpTransport`
(`python/parmana/transport/http_transport.py`) never sets an `Authorization` header. Against
any server with caller authentication enabled, the default and only supported production
configuration, see [Authentication](/api-reference/authentication), every request from this
SDK gets a `401`. There is no supported workaround today short of patching the transport
locally. This is real and current, not a documentation gap, tracked as a [FUTURE] item.
</Warning>
