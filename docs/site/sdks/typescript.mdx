...
(`AuthenticationError` and `AuthorizationError` were removed in a later
session: the Runtime enforces no authentication or authorization, so a
client-side taxonomy implying it does was misleading, see docs/CLAIMS.md.)
