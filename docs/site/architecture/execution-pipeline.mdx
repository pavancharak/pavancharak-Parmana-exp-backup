...
| 6. Credential Isolation / Connector Enforcement | Resolves a credential handle inside the Gateway boundary, then forwards verified content to the target system through a capability-checked Connector | [AVAILABLE] (legacy `HttpConnector`; new capability-aware `HttpConnector`/`MockConnector` via `@parmana/connector-sdk`) | `packages/execution-gateway/src/HttpConnector.ts`, `packages/connector-sdk/src/{HttpConnector,MockConnector,SdkConnectorExecutor}.ts` |
