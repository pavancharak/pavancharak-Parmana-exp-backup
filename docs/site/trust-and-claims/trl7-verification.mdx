...
| The original live-money proof (a real Razorpay test-mode refund, idempotency, policy denial) | Not re-run. Rested on a `CLAIMS.md` section documenting the original Razorpay proof, removed in full along with the connector on 2026-08-12 — see `docs/site/changelog.mdx` for the dated commit record; no current `CLAIMS.md` section describes this specific evidence | attested, not re-verified |
| Deployed-environment behavior (real webhook delivery, settlement closure, live-mode operation on Fly.io) | Not re-checked. Rests on `CLAIMS.md`'s historical §3.8/§3.9 (marked "Historical: Razorpay connector removed 2026-08-12") — the intermediate milestones §3.6/§3.7 were removed in full, not merely marked historical; see `docs/site/changelog.mdx` for their dated record | attested, not re-verified |
