...
<Info>
**[AVAILABLE]**, every command below was re-run against a live local server on 2026-07-29
(commit `0e69ed4`), including `python/examples/quickstart/run.py` itself.
</Info>

<Warning>
**Step 6's transaction now declares a `vendorId` signal it didn't need before.**
...
for what it was. Before this fix, this same request executed successfully
without `vendorId`; it no longer does.
</Warning>
...
will fail closed against this repo's demo Supabase credentials otherwise. The one connector
wired into the default server, `vendor-payment`, is a reference mock that reads its
credential from an environment variable, set a placeholder value for it:
...
Captured from an actual run, 2026-07-29 (IDs and hashes differ on every run):
