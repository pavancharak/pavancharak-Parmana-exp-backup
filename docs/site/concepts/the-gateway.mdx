...
<Warning>
**Only one connector is registered in the default server today**: `vendor-payment`
(`packages/api/src/bootstrap/createVendorPaymentConnector.ts`, a `MockConnector` under
capability `"payments:execute"`). The four enterprise mocks above exist in
`@parmana/connector-sdk` and are unit-tested there, but nothing in
`packages/api/src/bootstrap` registers them, `POST /execute` with any action other than
`"payments:execute"` fails closed with `"No connector registered for action"`. Adding a
connector to the running server is a bootstrap code change today (`createConnectorRoute.ts`,
