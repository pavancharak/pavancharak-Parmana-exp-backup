...
<Info>**[AVAILABLE]**, validated this session against a real, permanent Fly.io deployment (`parmana-api`), plus a second deployment on the same platform running live-mode Razorpay credentials. Platform-agnostic by design, built against a bare `docker build` / `docker run`, not a specific PaaS's proprietary build system. Source: `DEPLOYMENT.md` (repo root).</Info>
...
live-mode operation, or failover, running multiple machines is for redundancy, not capacity or
