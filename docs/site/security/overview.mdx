...
## Caller authentication exists; per-caller authorization does not

`packages/api` gates every route except `GET /health` behind a caller bearer key, verified
live this pass, see [Authentication](/api-reference/authentication). What's still missing:
any authenticated caller can call any route (no per-caller scoping), there is no mutual TLS
by default, and rotation is a manual operator procedure with no self-service endpoint, see
[Deploy patterns](/guides/deploy-patterns#caller-authentication). This authentication layer
is independent of, and does not substitute for, [Execution
