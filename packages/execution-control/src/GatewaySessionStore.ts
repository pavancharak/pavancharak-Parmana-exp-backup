...
import type { ExecutionRelease, GatewayExecutionRequest, GatewaySession } from "./types.js";
...
export class InMemoryGatewaySessionStore {
  ...
  create(
    ...
    if (issuanceAuthentication !== this.issuanceAuthentication) {
      throw new Error("Gateway session issuance rejected unauthenticated caller.");
    }
    const authorizationExpiry = Date.parse(release.authorization.payload.expiresAt);
    const expiresAt = new Date(Math.min(now.getTime() + lifetimeMs, authorizationExpiry));
    const record: SessionRecord = {
      sessionId: randomUUID(),
      createdAt: now.toISOString(),
      expiresAt: expiresAt.toISOString(),
      authorizationId: release.authorization.payload.authorizationId,
      connectorId: release.connectorName,
  ...
  consume(request: GatewayExecutionRequest, connectorId: string, now = new Date()): boolean {
    const record = this.sessions.get(request.gatewaySession.sessionId);
    const valid = record !== undefined && !record.used &&
      Date.parse(record.expiresAt) > now.getTime() &&
      record.connectorId === connectorId &&
      record.authorizationId === request.authorization.payload.authorizationId &&
    ...
    if (!valid || record === undefined) return false;
    record.used = true;
    return true;
  }

  private sameSession(a: GatewaySession, b: GatewaySession): boolean {
    return a.sessionId === b.sessionId && a.createdAt === b.createdAt &&
      a.expiresAt === b.expiresAt && a.authorizationId === b.authorizationId;
  }
