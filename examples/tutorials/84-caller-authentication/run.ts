...
try {
  ...
  const rawKeyLeaked = serialized.includes(CALLER_A_KEY) || serialized.includes(CALLER_B_KEY);
  ...
  if (allPassed) {
    console.log(
      "✓ Authentication gates every route but /health as expected, key rotation works, and the raw key never appears in the audit trail.",
    );
  } else {
